...
## v3.0 の変更（UI 強化）
1. アイコン表示: インストール済みのアプリは PackageManager から実物のアイコンを取得して
   一覧に表示（AppIcons.kt、メモリキャッシュ）。未インストールは頭文字を表示する。
   Release に画像アセットを増やさない方針のため、この方式を採った。
2. 並び順: catalog.json の apps[].order（数値、小さい順）で並ぶ。未設定は 9999 扱いで最後尾。
3. ステータス: catalog.json の apps[].status（テスト中 / 修正中 / 最終版 / 停止中 / なし）。
   設定されているとアプリ名の横にチップで表示される。
4. トップバーを整理。「まとめて」と「メニュー」の2つにし、メニュー内に
   「一覧を再読み込み」「並び順・ステータス管理」「トークン設定」を格納。
5. 管理画面（メニュー → 並び順・ステータス管理）で番号入力とステータス切替を行い、
   保存すると CatalogWriter.kt が GitHub API 経由で catalog.json を直接更新する。
   端末内ではなくカタログに保存されるため、別端末でも同じ並び順・ステータスになる。
   ※ カタログを書き換えるので、トークンには対象リポジトリへの write 権限が必要。
